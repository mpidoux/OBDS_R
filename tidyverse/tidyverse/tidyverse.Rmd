...
> Answer: it tells us the the delimiter is a , and that there are 150 rows and 5 columns
...
> Answer: 150rows x 5columns, most of the columns are dbl (which is a number with a decimal point), the species column is a chr 

- Write the dataset to a file named `iris.tsv`, separating fields with the tabulation character.

```{r}

write_tsv(iris_raw, "irisfile.tsv")
...
##Note accross only works if it has a function before it - it applies the function before it ACROSS the columns you have specified. In this case we have told it to apply the function accross the columns which are numeric, and told it to calculate the means of these.
...
iris_long2 <- 
   iris_raw %>% 
  mutate(ID = paste0(species, seq(1, nrow(iris_raw)))) %>% 
  pivot_longer((where(is.numeric)))
    
##alternatively can be done using the code below
iris_long2 <- iris_raw %>%
  mutate(
...
exprs_assay_long <- exprs_assay %>% 
  pivot_longer(cols = where(is.numeric), names_to = "sampleID")
