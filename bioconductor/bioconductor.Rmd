...
Don't access internal slots directly. Just because this is not bioconductor best practice. Because if the developer adds funuctionality you could lose it by writing your code like this. 
...
> Answer:
> The output of this code tells us that we have 30 packages that are out of date, it helpfully gives us a line of code to use to update these packages if we want. As we are working on the cluster we dont want to update these packages however. 
...
```{r, eval=FALSE}
##we can click through options in the packages tab to find this

##the code to find it would be browsevignettes() but this doesnt work on the cluster
```
...
##to extract an element from a list you can use a $ sign or [[]]
```
...
## by NN in our DNA sequence we mean accept anything in these positions. The default fixed = TRUE for the countPattern function it is looking actually for Ns but we want it to mean anything, so you need to change it to fixed = FALSE
```
...
##mcols refers to the metadata columns
```
...
?unique
transcripts_actb <- unique(gtf_ACTB$transcript_id) 

##Then we need to remove NAs

transcripts_actb2 <- transcripts_actb[!is.na(transcripts_actb)]



```

- Subset the actb annotations to exons, and use the `split()` function to separate exons from different transcripts.
...
##Returns the number of exons within each transcript_ID
```
...
##Entrezid often used for pathway analysis 
```
...
# Exercise
The metadata bit can store anything without dimensions, includes notes etc
...
##Usually the top section of this you would be reading in data files, but here we are making up some data
```
